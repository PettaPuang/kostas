...
import { getPendingExtensionRequestsCount } from "@/lib/actions/subscription-message.actions";
...
export function SpbuList({
  ...
  const [pendingExtensionRequestsCount, setPendingExtensionRequestsCount] = useState(0);
  ...
  // Load pending extension requests count for developer
  useEffect(() => {
    if (isDeveloper) {
      const loadPendingCount = async () => {
        try {
          const result = await getPendingExtensionRequestsCount();
          if (result.success && result.data !== undefined) {
            setPendingExtensionRequestsCount(result.data);
          }
        } catch (error) {
          console.error("Failed to load pending extension requests count:", error);
        }
      };
      loadPendingCount();
      ...
      {isDeveloper && (
        <div className="border-t bg-blue-50/50">
          <Button
            ...
            variant="ghost"
            className="w-full p-2 lg:p-4 flex items-center justify-between hover:bg-blue-100/50 transition-colors"
          >
